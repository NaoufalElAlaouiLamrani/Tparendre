...
class Vehicle {
  ...
  wander(obstacles) {
    ...
    // Vérification des bordures avec une marge plus grande
    let margin = this.r * 4; 
    // Marge élargie
    if (this.pos.x < margin) {
      // Pousse vers la droite
      steer.add(createVector(this.maxForce, 0)); 
    }
    ...
    if (this.pos.y < margin) {
      // Pousse vers le bas
      steer.add(createVector(0, this.maxForce)); 
    }
  ...
  applyBehaviors(target, obstacles, vehicules) {
    if (!target) {
      console.error("Target is undefined in applyBehaviors");
      return;
    }
  
    let seekForce = this.arrive(target); // Utilise arrive pour un ralentissement fluide
    let avoidForce = this.avoid(obstacles);
    let separateForce = isSnakeMode ? createVector(0, 0) : this.separate(vehicules);
  ...
  arrive(target, distance = 0) {
    if (!target) {
      console.error("Target is undefined in arrive method");
      return createVector(0, 0);
    }
  
    let targetPos = target instanceof p5.Vector ? target : target.pos;
    ...
    if (dist > distance) {
      if (dist < this.rayonZoneDeFreinage) {
        desiredSpeed = map(dist, distance, this.rayonZoneDeFreinage, 0, this.maxSpeed);
      }
    } else {
      desiredSpeed = 0;
    }
  ...
  */
  ...
  update() {
    // Ajuster la vitesse maximale en fonction du mode snake
    if (isSnakeMode) {
      this.maxSpeed = 3; // Réduire la vitesse en mode snake
    } else {
      this.maxSpeed = 6; // Rétablir la vitesse normale sinon
    }
  
    // Ajouter l'accélération à la vitesse
    this.vel.add(this.acc);
    // Contraindre la vitesse à la valeur maxSpeed
    ...
    // Mise à jour du chemin (trace derrière le véhicule)
  ...
  followLeader(leader) {
    // Position cible : position du leader avec un léger décalage
    let target = leader.pos.copy();
    let offset = leader.vel.copy();
    offset.setMag(leader.r * 2); // Ajuster la distance entre les véhicules
    target.sub(offset);
  
    // Si le leader est arrêté, arrêter ce véhicule également
    if (leader.vel.mag() < 0.1) {
      // Comportement arrive pour s'arrêter progressivement 
      return this.arrive(target); 
    } else {
      // Sinon, comportement seek pour suivre le leader
      return this.seek(target); 
    }
  }

    // Méthode pour vérifier la collision avec les projectiles
    checkCollision(projectiles) {
      return projectiles.some(p => p5.Vector.dist(this.pos, p.pos) < this.r + p.r);
    }

  fleeProjectiles(projectiles) {
    let fleeForce = createVector(0, 0);
  
    projectiles.forEach(p => {
      let d = p5.Vector.dist(this.pos, p.pos);
      // Distance à laquelle le véhicule détecte le projectile
      if (d < 200) { 
        let fleeVec = p5.Vector.sub(this.pos, p.pos);
        // Plus le projectile est proche, plus la force de fuite est grande
        fleeVec.setMag(1 / d * 500); 
        fleeForce.add(fleeVec);
      }
    });
...
class Predator extends Vehicle {
  ...
  shoot() {
    if (this.timeSinceLastShot >= this.shootInterval) {
      let projectile = new Projectile(this.pos.x, this.pos.y, this.vel.copy());
  ...
  update() {
    ...
    this.projectiles = this.projectiles.filter(p => !p.isOutOfBounds());
  }

  show() {
    // Affichage du prédateur (rouge)
    push();
    stroke(255, 0, 0);
    strokeWeight(2);
    fill(255, 0, 0);
...
class Projectile {
  ...
  show() {
    // Affichage des projectiles (jaunes)
